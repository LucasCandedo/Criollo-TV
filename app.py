...
# Solo importar flet_video si no estamos en web
try:
...
async def main(page: ft.Page):
    ...
    async def reproducir_canal(nombre_canal: str):
        ...
        # -------------------------
        # WEB: iframe de YouTube
        # -------------------------
        if es_web:
            print(f"Modo web: usando iframe para video_id={video_id}")

            player = fwv.WebView(
                url=f"https://www.youtube.com/embed/{video_id}?autoplay=1&controls=1",
                expand=True,
            )
